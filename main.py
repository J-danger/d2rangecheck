...
class TransparentBox(tk.Tk):
    ...
    def capture_text(self):
        ...
        self.processed_text = first_line.replace('@', 'O').replace('®', 'o')

        # Show the processed text in a new window
        self.show_text(self.processed_text)

        # Find matching items
        matching_items = find_matching_items(self.processed_text)
        cleaned_items = clean_matching_items(matching_items)
        print(cleaned_items,'this one')  # Do something with cleaned items
    ...
    with open(filename, 'r') as file:
        ...
        for item_dict in items_data:
            for item_name, attributes in item_dict.items():
                if item_name in processed_text:
